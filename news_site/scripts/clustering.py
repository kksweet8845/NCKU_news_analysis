...
from newsdb.models import New, cluster_day, cluster_week, cluster_week_brand
...
def run():
    ...
    news_query_week = New.objects.filter(Q(date__gte=(date.today()-timedelta(days=6)).isoformat()) & Q(date__lte=date.today().isoformat()))
    text = tf.Variable(np.empty((0, 512)), dtype=np.float32)
    for i in tqdm(range(int(len(news_query_week)/100) + 1)):
        temp = news_clustering.getEmbed(news_query_week[(i*100):((i+1)*100)])
        text = tf.concat((text, temp), axis=0)
    top_news = news_clustering.getTopNews(text)
    top_news.sort(key=len, reverse=True)
    ...
    for news_list in tqdm(top_news):
        for news in news_list:
            a = cluster_week(news=news_query_week[news], date=news_query_week[news].date, 
                            cluster=cluster_no, 
    ...
    brands = [12, 16]
    for brand in brands:
        news_query_week_brand = New.objects.filter(Q(date__gte=(date.today()-timedelta(days=6)).isoformat()) 
                                             & Q(date__lte=date.today().isoformat())
                                             & Q(brand__exact=brand))
        text = tf.Variable(np.empty((0,512)), dtype=np.float32)
        for i in tqdm(range(int(len(news_query_week_brand)/100) + 1)):
            temp = news_clustering.getEmbed(news_query_week_brand[(i*100):((i+1)*100)])
            text = tf.concat((text, temp), axis=0)
        top_news = news_clustering.getTopNews(text)
        top_news.sort(key=len, reverse=True)
        ...
        for news_list in tqdm(top_news):
            for news in news_list:
                a = cluster_week_brand(news=news_query_week_brand[news], date=news_query_week_brand[news].date, 
                                       cluster=cluster_no, brand=brand,
                                       date_from=(date.today()-timedelta(days=6)).isoformat(),
                                       date_to=date.today().isoformat())
                a.save()
            cluster_no += 1
