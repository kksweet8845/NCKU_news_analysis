...
export default function PubCircle(props) {

    useEffect(() => {
        if(props.data){
            let preData = []
            Object.keys(props.data).map( brand => {
                let num = 0
                Object.keys(props.data[brand]).map( keyword => {
                    num += props.data[brand][keyword].tally
                })
                preData.push({
                    'title': brand,
                    'tally': num
            ...
            console.log(preData)
            circleRender(preData, props.id, props.treshold)
        }
    }, [props.data])
